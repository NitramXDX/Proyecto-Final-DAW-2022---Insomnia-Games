...
const username = /^[A-Za-z0-9]*$/; //Letras, numeros convencionales sin espacios y caracteres especiales
...
//	mensaje.style.display = 'none';
	...
	case "password":
		status = pass.test(e.target.value);
		msj = "ERROR: Contraseña no válida."
		change(status, e.target, msj);
        break;
	
	case "password2":
		status = pass.test(e.target.value) && e.target.value==document.querySelector("#password").value;
